...
def download_video(url, output_dir='.'):
    try:
        ...
        ydl_opts = {
            'format': 'bestvideo[ext=mp4][height<=1440]+bestaudio[ext=m4a]/best[ext=mp4][height<=1440]',  # Best video and audio available, up to 1440p resolution
            ...
            'postprocessors': [],  # No post-processing (no conversion)
            'noconvert': True,  # Don't convert formats after download
            'max_filesize': None,  # No filesize limit
        }
