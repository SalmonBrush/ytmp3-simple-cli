...
def convert_to_mp3(input_path, output_path):
    try:
        # Command to convert the audio to mp3 using FFmpeg
        command = ['ffmpeg', '-i', input_path, '-vn', '-ar', '44100', '-ac', '2', '-b:a', '192k', output_path]
        subprocess.run(command, check=True)
        print(f"{Fore.GREEN}Conversion successful: {output_path}")
    except subprocess.CalledProcessError as e:
        print(f"{Fore.RED}Error during conversion: {e}")
    except Exception as e:
        print(f"{Fore.RED}An unexpected error occurred during conversion: {e}")


# Function to download YouTube video (or playlist) using yt-dlp with a custom progress bar
def download_video(url, output_dir='.'):
    try:
        # Options for downloading audio only in the best format
        ydl_opts = {
            ...
            if 'entries' in info_dict:  # This means it's a playlist
                ...
                with yt_dlp.YoutubeDL(ydl_opts) as playlist_ydl:
                    ...
                    for idx, entry in enumerate(info_dict['entries'], start=1):
                        entry['ext'] = 'm4a'  # Ensure the extension is set to audio format (prefer m4a)
                    ...
                    for idx, video_url, full_path in entries_to_download:
                        ...
                        try:
                            ...
                            temp_file = full_path  # This is the downloaded file
                            mp3_file = os.path.splitext(temp_file)[0] + '.mp3'
                            convert_to_mp3(temp_file, mp3_file)  # Convert it to MP3

                            # Delete the original downloaded file
                            os.remove(temp_file)
                            print(f"{Fore.RED}Deleted temporary file: {temp_file}")
                        except Exception as e:
